...
public class Vehicle : MonoBehaviour{
	...
	public float tempSpeed = 10f;

	// Vehicle components


	Transform wheelFrontLeftHolder, wheelFrontRightHolder;


	Transform container, wheelFrontLeft, wheelFrontRight, wheelRareLeft, wheelRareRight;
	Transform body;
	...
	void Awake(){
		...
		foreach(Transform t in GetComponentsInChildren<Transform>()){
			...
			switch(t.name){
				...
				case "FL": wheelFrontLeft = t; break;
				case "FR": wheelFrontRight = t; break;
				case "RL": wheelRareLeft = t; break;
				case "RR": wheelRareRight = t; break;
				case "FL_holder": wheelFrontLeftHolder = t; break;
				case "FR_holder": wheelFrontRightHolder = t; break;
	...
	void Update(){
		...
		if(motorcycleTilt){ tilt = -rotateTarget / 1.5f; }
		
		container.localPosition = containerBase + new Vector3(0, Mathf.Abs(tilt) / 1000, 0);
		...
		if (wheelFrontLeftHolder != null) { wheelFrontLeftHolder.localRotation = Quaternion.Euler(0, rotateTarget / 2, 0); }
		if (wheelFrontRightHolder != null) { wheelFrontRightHolder.localRotation = Quaternion.Euler(0, rotateTarget / 2, 0); }

		wheelFrontLeft.transform.Rotate(speedTarget * 10 *Time.deltaTime, 0, 0);
		wheelFrontRight.transform.Rotate(speedTarget * 10 * Time.deltaTime, 0, 0);
		wheelRareLeft.transform.Rotate(speedTarget * 10 * Time.deltaTime, 0, 0);
		wheelRareRight.transform.Rotate(speedTarget * 10 * Time.deltaTime,0, 0);
	...
	void FixedUpdate(){
		...
		sphere.AddForce(vehicleModel.forward * tempSpeed);
		sphere.AddForce(Vector3.down * gravity, ForceMode.Acceleration);


		transform.position = sphere.transform.position + new Vector3(0, _offset, 0);
